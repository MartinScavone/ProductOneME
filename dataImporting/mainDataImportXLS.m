...
%%Matlab-only imports - uses XLSREAD function on "dataImport" file (defined from the main script
...
%exportToPDF = runVerbose(2);                         %boolean variable. if = 1, make pdf files with the many graphs to use. Use exportPDF (dependency:: GhostScript). Disabled in this 1st release of Product-One
...
saveDataName = string(saveDataName); %name of the output files  (code will produce a .mat file with the calculation results and pdf files with graphs and many reports)
